...
export function cloneLayerStyleStack(
...
export function supportsLayerStyles(
  layer: Pick<LayerNodeMeta, "layerType"> | null | undefined,
): boolean {
  return (
    layer?.layerType === "pixel" || layer?.layerType === "text" || layer?.layerType === "vector"
  );
}
