...
- [ ] Cover the matrix that actually exercises the parser — **27 of 30 capabilities covered, 23 fixtures, ~125 KB of binaries** (2026-09-17: added `structure.group-empty`, `clipping.across-group-boundary`, `mask.larger-than-layer`, `mask.on-group` and `fill-opacity.partial`; 2026-09-18: added `compression.zip` and `compression.zip-prediction`); `manifest.json` is the ledger and `TestCorpusMatrixCoverage` fails on an uncovered capability that is not explicitly deferred
  ...
  - [ ] Adjustment layers, text layers and layer effects — 3 capabilities still deferred; each carries a `deferred` PLAN.md reference in the manifest rather than being silently absent. All three need real descriptor payloads
