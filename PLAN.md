...
- [ ] Cover the matrix that actually exercises the parser — **28 of 30 capabilities covered, 26 fixtures, ~141 KB of binaries** (2026-09-17: added `structure.group-empty`, `clipping.across-group-boundary`, `mask.larger-than-layer`, `mask.on-group`, `fill-opacity.partial` and `layer.adjustment`; 2026-09-18: added `compression.zip` and `compression.zip-prediction`); `manifest.json` is the ledger and `TestCorpusMatrixCoverage` fails on an uncovered capability that is not explicitly deferred — (2026-09-17) — partial: `layer.text` and `layer.effects` remain, with re-checked reasons naming what is actually missing
...
- [x] **Parse the genuine adjustment blocks into live `AdjustmentLayer`s — done 2026-09-17** for all 12 named here: `levl`, `curv`, `hue2`, `brit`, `blnc`, `blwh`, `mixr`, `phfl`, `selc`, `thrs`, `post`, `nvrt`. New `internal/io/psd/adjustments.go` reconstructs each into the engine's existing adjustment kind plus a JSON parameter object, and `psdimport` builds a `model.AdjustmentLayer` before the raster branch is tried. No model change was needed: `AdjustmentLayer` is already `AdjustmentKind` + an opaque `json.RawMessage`. **The solid/gradient/pattern fill types are excluded and stay open** — see the new item below; the engine has no fill-layer type at all, and inventing one is a model decision, not a parser one
  ...
  - [x] Values with no engine home are reported, never rounded into a field that means something else: `levl`'s per-channel records (the engine holds one set of level values), `mixr`'s constant term, `phfl`'s non-RGB colour spaces, `hue2`'s four movable hue edges. `curv` version 4 is refused outright because it drops the channel bitmap, so a red curve could only be guessed onto the composite
