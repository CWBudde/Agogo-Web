...
- [x] Add `Fuzz*` targets for `Parse`, `ParseLayerAndMaskInfo`, `ParseCompositeImageData`, `DecodePackBits`, and the descriptor reader; seed the corpus from S.10.1 fixtures and check in any crashers found
  ...
  - [ ] `FuzzParseDescriptorTextValue` still runs on synthetic seeds only — the corpus genuinely has no text descriptor, and will not until S.10.7 lands a text fixture
  - [x] `Fuzz*` now run as real mutation passes in CI, not only as seed execution: a separate parallel `go-fuzz-smoke` job runs all six targets at `-fuzztime=30s`. Not `-race` — `-fuzz -race` manages ~3.5k execs/s against ~70k, and race coverage of these paths already comes from the existing `-race` step executing every seed
