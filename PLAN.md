...
- [ ] Cover the matrix that actually exercises the parser — **24 of 30 capabilities covered, ~115 KB total** (2026-09-17: added `structure.group-empty`, `clipping.across-group-boundary`, `mask.larger-than-layer` and `mask.on-group`); `manifest.json` is the ledger and `TestCorpusMatrixCoverage` fails on an uncovered capability that is not explicitly deferred
