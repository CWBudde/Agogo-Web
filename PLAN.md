...
- [ ] Cover the matrix that actually exercises the parser — **26 of 30 capabilities covered, 22 fixtures, ~118 KB of binaries** (2026-09-17: added `structure.group-empty`, `clipping.across-group-boundary`, `mask.larger-than-layer` and `mask.on-group`; 2026-09-18: added `compression.zip` and `compression.zip-prediction`); `manifest.json` is the ledger and `TestCorpusMatrixCoverage` fails on an uncovered capability that is not explicitly deferred
  ...
  - [ ] Adjustment layers, text layers, layer effects, and fill opacity (`iOpa`) — 4 capabilities still deferred; each carries a `deferred` PLAN.md reference in the manifest rather than being silently absent. `iOpa` is the cheap one and is **not** blocked (see S.10.7); the other three need real descriptor payloads
...
- **`FillOpacity` is never read from or written to PSD** — `iOpa` appears nowhere in the codebase and `newLayerBase` defaults it to 1. Two-sided gap. (S.10.7) — **but it is not blocked on a fixture**: `derive_expectations.py` already emits `fillOpacity255`, `compare.go` already asserts it against `node.FillOpacity()`, and pytoshop can write the block with `GenericTaggedBlock(code=b'iOpa', data=bytes([128]))`. Only the engine's read/write side is missing.
- **The remaining flattening path still double-applies what it bakes in.** A layer with enabled layer styles or a non-identity BlendIf is still rendered through `RenderLayer` and cropped, because neither has a faithful PSD encoding today. That flattened raster has the layer's mask, clipping, opacity *and* styles already baked into it, yet `psdexport` still writes the `-2` mask channel, the clipping byte, the opacity byte and an `lfx2` effects block — so every one of them is applied a second time on re-read. Found while fixing the mask/clipping defects above; left recorded rather than fixed because no fixture can currently exercise it (`layer.effects` is a deferred capability, and no available writer emits effects) and because the honest fix is to give styles and BlendIf real PSD encodings rather than to choose which copy to drop. (S.10.7)
...
- [ ] **Cheapest item first — `iOpa` fill opacity is a small two-sided fix, not a Photoshop dependency.** The model already carries `FillOpacity`, the deriver already emits `fillOpacity255` and `compare.go` already asserts it; the fixture is writable today via `GenericTaggedBlock(code=b'iOpa', data=bytes([128]))`. What is missing is only parsing `iOpa` into `FillOpacity` on import and emitting it on export. Doing this closes the last non-descriptor capability in the S.10.1 matrix
  - [ ] pytoshop gotcha for whoever writes that generator entry: `GenericTaggedBlock.data`'s **setter is a no-op** — it validates `isinstance(val, bytes)` and then never assigns. Pass `data` to the constructor, never by assignment
