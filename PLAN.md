...
- [ ] **Finding (2026-09-18, from PR review) — the adjustment compositor ignores opacity and blend mode.** `applyAdjustmentLayerToSurface` takes its coverage from the clip alpha and the layer mask only; `Opacity()` and `BlendMode()` are never read, so a visible adjustment layer at opacity 160 with Multiply renders full-strength Normal. This is a **renderer** gap, not an I/O one — the same shape as the group fill-opacity finding above — and it predates PSD import: a layer created in the UI has always behaved this way. Import carries both values faithfully, and the fixture's Levels layer pins them at model level. Fixing it means threading opacity and a blend function through the adjustment path and its dirty-rect cache, plus a visible non-default case in the render tests
