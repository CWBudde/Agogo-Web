...
- [ ] Cover the matrix that actually exercises the parser — **28 of 30 capabilities covered, 26 fixtures, ~142 KB of binaries** (2026-09-17: added `structure.group-empty`, `clipping.across-group-boundary`, `mask.larger-than-layer`, `mask.on-group`, `fill-opacity.partial` and `layer.adjustment`; 2026-09-18: added `compression.zip` and `compression.zip-prediction`); `manifest.json` is the ledger and `TestCorpusMatrixCoverage` fails on an uncovered capability that is not explicitly deferred — (2026-09-17) — partial: `layer.text` and `layer.effects` remain, with re-checked reasons naming what is actually missing
  - [x] 8-bit RGB and grayscale; RAW, RLE and ZIP (composite *and* layer channels); nested groups; open vs closed folders; pass-through vs isolated groups; layer masks with non-zero offsets, disabled and inverted; clipping masks; all 27 blend modes; PSB; a PSD exactly on the 30000 px limit; a 16-bit file asserted to be *rejected*
  - [x] ZIP and ZIP-with-prediction (2026-09-18) — **the `deferred` reason was false, not the capability**. It read "no available writer emits them; pytoshop offers raw and RLE only". In fact `pytoshop.codecs.compress_zip` is pure `zlib` and never touches the unbuilt Cython `packbits` extension, so ZIP layer channels were writable the whole time; only `compress_zip_prediction` needed a workaround, and it is the same shape as the `_PackBits` one already in the generator. A `deferred` entry is a claim about the world and has to be re-checked, not inherited — recorded in `testdata/README.md` as well
  - [x] Fill opacity (`iOpa`) — covered by `rgb8-fill-opacity` (2026-09-17). **The second `deferred` reason to be found false, after ZIP**: it read "needs a Photoshop-authored file. iOpa is Photoshop-only, and Agogo neither reads nor writes it, so the gap is two-sided". `GenericTaggedBlock` writes the block in three lines, and "Agogo neither reads nor writes it" was a description of the very gap the corpus exists to expose, not a reason to leave it unexposed
  - [x] Adjustment layers (2026-09-17) — **the third `deferred` reason to be found false, after ZIP and `iOpa`**. It read "needs a Photoshop-authored file. No available writer emits live adjustment layers (levl/curv/hue2 and friends)", which described pytoshop's typed convenience classes rather than the world: `GenericTaggedBlock` writes any tagged block, and a Photoshop-shaped adjustment layer is just a layer record with an empty rect, no colour channels and the block. Three fixtures — `rgb8-adjustment-core` (levl, curv, hue2), `rgb8-adjustment-binary` (blnc, mixr, selc, thrs, post, nvrt, phfl) and `rgb8-adjustment-descriptor` (blwh, brit + CgEd) — cover 12 block types, and psd-tools reads every one back as its native adjustment class. Payloads are hand-built from the spec in the generator on purpose: psd-tools is the *deriver*, and a corpus whose generator and deriver are the same implementation asserts only its own self-consistency
  - [ ] Text layers and layer effects — 2 capabilities still deferred, each with a re-checked reason naming the specific obstacle rather than "only Photoshop can write it": `layer.text` needs `TySh`'s undocumented text-engine data (the descriptor around it *is* writable by hand); `layer.effects` is writable as a descriptor but is waiting on the **reader**, since the S.5 style model has no contour, glow-technique, glow-source or gradient-type field to assert against
...
- [x] Add the writer half: `TestPSDFixtureReexportSurvivesSelfReimport` re-exports every fixture, compares the re-import **against the external expectation** (not against the pre-export document), and dumps the bytes to `_dump/` for `just fixtures-verify`, which re-reads them with psd-tools and ImageMagick. Last run: 25/25 accepted (every fixture but the negative `depth16-rejected`, which has no re-export). Logged per fixture in `testdata/VERIFICATION.md`
  - [x] The re-export is checked at **two** levels, because the first alone is not enough: `CompareReexport` sees the reconstructed document, which never holds section-divider types, mask rectangles, mask default fill or channel IDs, so a writer could corrupt all of them and stay green — `rgb8-group-closed-folder` was exactly that false pass. `CompareReexportRecords` reparses the re-exported bytes and holds the flat records to the same external `psdRecords` expectation the reader is held to
  - [x] All 27 blend keys survive Agogo's *write* byte-exact, trailing spaces included; `pass` survives for pass-through groups
  - [x] Raw `psdBlendKey` is asserted at record scope: `LayerRecord.BlendKey` keeps the four-character key exactly as read, trailing spaces included, and the writer leg holds the re-export to it. `blendMode` alone cannot: `norm` and `pass` both normalise to Normal, so a writer emitting either for the other passes every model-scope assertion. All 27 blend keys now demonstrably survive the round trip rather than being asserted only by claim
  - [x] An allowlist entry may excuse a known *difference*, never the loss of the data that difference is a spelling of: invariants like the channel-ID set are emitted strictly and are unreachable from `writer.lossy`. Found by review — the ordered `channelIds` entry was suppressing the whole slice, so a dropped `-2` user mask passed
  - [x] `writer.lossy` is checked for completeness, not just correctness: `CompareReexportAll` runs both legs in one pass and fails any allowlist path that no assertion addresses. A typo'd or outlived entry excuses nothing and can never be reported stale, so without this it reads as a reviewed exemption while doing nothing
...
- [x] Add a golden-warning assertion per fixture — `warnings` is an exact order-insensitive set per sidecar; all 23 fixtures currently import with zero warnings
- [x] Keep fixture size bounded — `TestCorpusSizeBudget` enforces 128 KiB per file and 1 MiB total against soft targets of 24 KiB / 512 KiB; the corpus is ~125 KB of binaries across 23 files
...
- ~~**`SavePSD` destroys pixel data on clipped layers.**~~ **Fixed 2026-09-17** (S.10.3). A clipped layer's stored bounds were cut to the intersection with its clip base — (8,6) 18x14 re-exported as (8,9) 11x11 — so content outside the base was gone on save.
- ~~**Re-export trims a layer to its mask rect when the mask is inverted**~~ **Fixed 2026-09-17** (S.10.4), so an inverted mask is non-destructive again.
- ~~**Mask attenuation is not bit-exact across a round trip.**~~ **Fixed 2026-09-17** (S.10.4).
  - All three were one defect: `psdexport` flattened masks and clipping into the stored raster and then cropped to the opaque bounding box, while *also* writing the mask channel and the clipping byte — so the data was destroyed on write and what survived was attenuated twice on read. Layer pixels are now written verbatim; only layer styles and BlendIf, which still have no faithful PSD encoding, are flattened.
- **The open/closed folder flag is lost at import.** The parser reads `lsct` 1 vs 2 correctly — `rgb8-group-closed-folder` asserts it at record scope — but the engine model has nowhere to store expanded state, so `psdexport` re-opens the closed folder on write. This is the S.10.3 item below, now backed by a file; the record leg of the writer check is what makes it fail rather than pass.
- **`psdimport` discards the mask rectangle and the invert flag.** The mask is rasterized to document size, so a re-export stores a full-canvas `mask.rect` instead of the original, and an inverted mask comes back as an equivalent non-inverted one. (S.10.4)
- **The `</Layer group>` bounding divider is written with the wrong blend key — or the fixtures are.** `newGroupEndRecord` hardcodes `pass` on the hidden `lsct` 3 record; pytoshop wrote `norm`. Which Photoshop writes is **not established** — no Photoshop-authored file is available and the spec does not prescribe it — so it is recorded rather than changed on a guess. `psdexport` is internally inconsistent here: `newGroupRecord` writes `pass` only for a non-isolated group, but the divider gets it unconditionally. The field is inert for readers, which identify the record by `lsct` 3. Resolve against a real Photoshop file. (S.10.3)
- **`psdexport` normalises layer channels.** Channels are emitted in a fixed order (`0,1,2,-1`; gray `0,-1`) with alpha always present, because the engine model is RGBA — so files authored with alpha first, or flat files with no alpha, differ on `channelIds`. The PSD spec does not prescribe channel order and readers key on the ID, so this is recorded per record rather than fixed. Because allowlisting a path suppresses the whole assertion, the ID *set* is asserted separately at `psdRecords[N].channelIds.preserved`, emitted strictly so no `writer.lossy` entry can reach it: it permits exactly one gained channel, the alpha `psdexport` always writes, and nothing else. A dropped `-2` user mask or a corrupted ID therefore still fails. (S.10.2)
- ~~**`FillOpacity` is never read from or written to PSD.**~~ **Fixed 2026-09-17** (S.10.7). `iOpa` appeared nowhere in the codebase, so fill opacity survived an Agogo↔Agogo `.agp` round trip and was silently lost through PSD in both directions. It is now parsed into `LayerRecord.FillOpacity`, applied on both the raster and the group import path, and written from `buildLayerExtraBlocks` — only below 255, because absence means 255, and as four bytes, because psd-tools reads `iOpa` as `B3x` and only falls back to a bare byte after logging an error. Covered by `rgb8-fill-opacity` and verified externally: psd-tools reads Agogo's re-export back as fill 128 and fill 0.
- **The remaining flattening path still double-applies what it bakes in.** A layer with enabled layer styles or a non-identity BlendIf is still rendered through `RenderLayer` and cropped, because neither has a faithful PSD encoding today. That flattened raster has the layer's mask, clipping, opacity, fill opacity *and* styles already baked into it, yet `psdexport` still writes the `-2` mask channel, the clipping byte, the opacity byte, the `iOpa` block and an `lfx2` effects block — so every one of them is applied a second time on re-read. `iOpa` joins the list deliberately rather than being suppressed alone, which would leave fill opacity right while plain opacity stayed doubled; `TestStyledLayerStillWritesFillOpacityOverTheFlattenedRaster` pins that choice. Found while fixing the mask/clipping defects above; left recorded rather than fixed because no fixture can currently exercise it (`layer.effects` is a deferred capability, and no available writer emits effects) and because the honest fix is to give styles and BlendIf real PSD encodings rather than to choose which copy to drop. (S.10.7)
...
- [x] **Update stale status:** RAW, RLE and ZIP are now all verified against external fixtures (`rgb8-flat-raw`, `rgb8-flat-rle`, `rgb8-rle-layers` for RAW/RLE — the last covering layer channels, not just the composite; `rgb8-zip-layers` and `rgb8-zip-prediction-layers` for ZIP, added 2026-09-18)
...
- [x] Validate `decodeZipChannel` / `decodeZipImageData` / `applyZipPredictionInPlace` (`pixels.go:140–207`) on real ZIP and ZIP-prediction fixtures, including the prediction reset at each row boundary — **done 2026-09-18, and all three passed unchanged**, the per-row reset included
  - [x] The fixture is not vacuous, which was checked rather than assumed: decompressing `rgb8-zip-prediction-layers` channel `-1` of record 0 and applying a *non-resetting* cumulative sum — the plausible bug — disagrees with the correct per-row decode on **736 of 768 bytes**, including both sampled bottom-row pixels (at (0,23) correct 255 vs broken 209; at (31,23) correct 254 vs broken 208). A decoder with that bug fails loudly against this file
  - [x] **Generator finding worth keeping:** pytoshop's `compress_zip_prediction` has a second, independent bug beyond the missing Cython extension — it passes the row as `row.flatten()`, which copies, while the real encoder mutates in place, so the prediction is computed and then discarded. Shimming only the missing encoder would have produced a file *labelled* `zip_prediction` carrying plain ZIP bytes — a fixture asserting nothing. The generator therefore replaces the compressor outright via `codecs.compressors[zip_prediction]` (the dict: `compress_image` dispatches through it, so rebinding the module-level name is silently ignored). Upstream `decompress_zip_prediction` has the identical `.flatten()` defect
  - [x] Sample-point finding: at the deriver's default `--max-samples 16` every sampled point for these fixtures landed on y=0, which the row reset cannot falsify. The fixtures are derived with `--max-samples 24` so each layer gets four corners including the bottom row
...
- [x] Add per-compression decode tests over fixtures and an encode→decode→compare test for each scheme — RAW, RLE and ZIP are all covered by the corpus. Note the two ZIP fixtures prove the **decoders**: `psdexport` picks its own channel compression on re-export, so they do not exercise a ZIP *writer*, and the sidecars' `lossyReason` says so
...
- [x] Preserve the open/closed distinction (type 1 vs 2) as group expanded state rather than discarding it — **done 2026-09-18**. `GroupLayer.Expanded` now carries it end to end: `psdimport.popStack` sets it from `record.SectionType`, `psdexport` writes `lsct` 1 or 2 instead of hardcoding 1, `ProjectLayerArchive.Expanded` persists it in `.agp`, and `LayerNodeMeta.expanded` reaches the Layers panel
  - [x] Archive field is `*bool`, not `bool`+`omitempty`: the model default is **true**, so `omitempty` would drop exactly the common case and reopen every folder on load. `nil` means "written before this field existed" and resolves to the default, so `ArchiveVersion` stays 1 and old `.agp` files load unchanged
  - [x] Toggling expansion is **not undoable** and does not mark the document dirty — it follows the `SetActiveLayer` precedent rather than `executeDocCommand`. Photoshop treats expansion as view state, and an undoable chevron would bury real edits in the history panel. It still persists, because the command mutates the document that both serializers read
  - [x] The panel's `collapsedGroups` is no longer local `useState` — it is derived from the layer meta, so a remount no longer reopens every folder and the map cannot drift from the engine
  - [x] The `psdRecords[6].sectionType` `writer.lossy` entry in `rgb8-group-closed-folder.expected.json` went stale and was removed; the fixture now asserts both `lsct` 1 and `lsct` 2 survive Agogo's write at record scope
- [x] Fix the writer side to match — verified against psd-tools; Photoshop itself remains unverified (no licence)
...
- [x] Add fixture tests for deep nesting, adjacent sibling groups, a group as the first/last layer, empty groups, and clipping across a group boundary
  ...
  - [x] `rgb8-group-empty` (2026-09-17) covers a genuinely childless group — a bounding divider immediately followed by its folder record — beside a populated group, so a reader that drops the group and one that swallows the next sibling fail differently. It round-trips correctly
  - [x] `rgb8-clipping-across-group` (2026-09-17) covers a clipped layer at the bottom of a group whose only candidate base lies outside it, with an in-group clip pair as the control
    - [x] **Finding:** clipping does not cross a group boundary, so the boundary-clipped layer has no base among its siblings and `normalizeGroupClipping` (`internal/engine/layer_ops.go`) clears its `ClipToBelow`. The parser reads the record's clipping byte correctly; the engine model then discards it, and the re-export writes `clipping=0`. Photoshop keeps the byte, so moving a layer underneath re-establishes the clip. Recorded as a `knownGaps` entry plus a `writer.lossy` entry rather than fixed: preserving a dormant clip flag means giving the model somewhere to keep it
    - [x] `compositePixels` is deliberately not asserted for this fixture — at (24,12) psd-tools' `composite()` renders the in-group clipped layer outside its base while Agogo correctly hides it, so Agogo is the one that is right and asserting agreement would pin Agogo to psd-tools' arithmetic
- [x] **Writer defect found by the corpus — fixed (2026-09-17):** `SavePSD` hard-clipped a clipped layer's stored bounds to the intersection with its clip base — `rgb8-clipping`'s (8,6) 18x14 layer was re-exported as (8,9) 11x11 — so pixels outside the base were destroyed on save
  - [x] Root cause: `canUseNativeLayerRaster` (`psdexport/export.go`) treated `clipToBelow` as a reason to flatten the layer through `RenderLayer`, which multiplies the layer's alpha by the clip base's alpha; `cropDocumentSurface` then shrank the stored bounds to the remaining opaque bounding box. The record *also* carried the clipping byte, so the clip was applied a second time on re-read
  - [x] Fix: a clipping flag no longer disqualifies a layer from writing its own stored pixels. PSD keeps the clipping byte beside an untouched raster and re-evaluates it when compositing, which is what Photoshop does
  - [x] Verified: the re-export now stores (8,6) 18x14 as re-read by psd-tools 1.19.0; the `layers[2].bounds`, `psdRecords[2].bounds` and `layerPixels[5]` allowlist entries went stale and were removed
...
- [x] Add fixture tests for offset masks, disabled masks, inverted masks, masks larger and smaller than their layer, and a mask on a group
  ...
  - [x] `rgb8-mask-larger-than-layer` (2026-09-17) covers a mask rect that strictly contains the layer rect (mask 24x20 at (4,2) over a 14x12 layer at (8,6)) with default fill 0, so a reader that sizes the `-2` channel from the layer rect under-reads it. It round-trips with the layer's own bounds intact
  - [x] `rgb8-group-mask` (2026-09-17) covers a raster mask on a group's opening (`lsct` 1) record attenuating both children, with an unmasked sibling as the control. The mask survives the round trip on the group record
  - [x] Both extend the existing `psdRecords[N].mask.rect` loss with `mask.defaultFill`: they are authored with fill 0, which `psdimport` folds into the rasterized document-sized mask, so the re-export stores 255. Coverage is unchanged; only the stored extent and fill byte differ
- [x] **Writer defects found by the corpus — fixed (2026-09-17):** re-export trimmed the layer to the mask rect when the mask was inverted (so an inverted mask stopped being non-destructive), and mask attenuation was not bit-exact across a round trip
  - [x] Root cause: the same `canUseNativeLayerRaster` bail as the S.10.3 clipping defect. A mask forced the layer through `RenderLayer`, which attenuated its alpha by the mask; `cropDocumentSurface` then trimmed the layer to whatever stayed opaque — for an inverted mask with default fill 255 that is exactly the mask rect. The mask was *also* emitted as the `-2` channel, so it was applied a second time on re-read, which is what darkened the sampled pixels
  - [x] Fix: a mask no longer disqualifies a layer from writing its own stored pixels; the mask travels as the `-2` channel alone, the way PSD represents it
  - [x] Vector masks, mask density and mask feather used to reach the file only by being baked into those pixels. `psdexport.Params` gained a `ResolveMask` callback, supplied by `internal/engine/psd_writer.go` from `Document.effectiveLayerMask`, so the resolved coverage is written into the `-2` channel instead of the raw `LayerMask`
  - [x] Verified: `rgb8-mask-inverted` re-exports at its full (4,3) 24x18 bounds instead of (10,7) 12x8, and `rgb8-layer-mask-offset`'s sampled blue reads 144 across the round trip instead of 92, both as re-read by psd-tools 1.19.0. Seven and one allowlist entries respectively went stale and were removed
  - [x] **Third defect found while fixing these two, also fixed: vector masks were dropped from PSD export entirely.** A vector mask sets `VectorMask`, not `LayerMask`, so `canUseNativeLayerRaster` never saw a mask, wrote the layer's raw pixels, and emitted no `-2` channel — the mask was neither baked in nor written out. `ResolveMask` now carries the vector coverage into the `-2` channel. No fixture could catch this: no available generator emits a vector mask, so it is pinned by `TestSavePSDWritesVectorMaskCoverage` in `internal/engine/psd_writer_test.go` instead
  - [x] Regression coverage: `internal/io/psdexport/export_nondestructive_test.go` pins the record shape (full bounds, byte-identical pixels, resolved `-2` channel, and that styled/BlendIf layers still flatten); `TestSavePSDKeepsMaskedAndClippedLayerPixelsIntact` pins the full engine round trip through `LoadPSDWithOptions(..., IgnoreEmbeddedProject: true)`
...
- [x] Audit the same pattern across all readers — section lengths (`readSectionLengthFrom`), channel lengths, descriptor strings, Unicode strings (`ParseUnicodeString`, `parseUnicodeStringFromReader`), Pascal strings, and the additional-layer-info block loop — **completed 2026-09-18, and it was not a formality: the audit found a live OOM (see the finding below)**
  ...
  - [x] `readBytesFrom` is now **fail-closed**: it bounds against `r.(interface{ Len() int })`, and a reader that does not implement `Len()` is an error instead of silently falling through to an unbounded read. All 30 call sites pass a `*bytes.Reader`, so this closes a latent hole rather than a live one
  - [x] `readSectionLengthFrom` deliberately bounds nothing — its contract is now documented in place, and the one caller that fed an allocation (`parser.go`, channel lengths) is bounded instead
  - [x] `ParseDescriptorTextValue`'s item loop is bounded by `reader.Len()/12` (4-byte key length + 4-byte key + 4-byte value type), matching the existing layer-count and channel-count guard shape. It terminated before, so this is consistency rather than a live DoS
  - [x] Channel lengths are validated per channel **and as a per-layer running sum**, against a snapshot of the remaining section taken before the channel table is consumed. Decision recorded: an overrun past the section end is an **error**, not a warning — channel data lives in the same section as the records, so a length past its end is a structural impossibility of the record itself, the same class as the two adjacent guards that already error. The existing warn-and-continue stays for the looser case it is actually good at: one malformed channel in an otherwise coherent record
...
- [x] Add `Fuzz*` targets for `Parse`, `ParseLayerAndMaskInfo`, `ParseCompositeImageData`, `DecodePackBits`, and the descriptor reader; seed the corpus from S.10.1 fixtures and check in any crashers found
  ...
  - [x] All 22 external fixtures now seed `FuzzParse`, and their extracted sections seed `FuzzParseLayerAndMaskInfo`, `FuzzParseCompositeImageData` and `FuzzParseLayerExtraData`, on every ordinary `go test` (`internal/io/psd/fixture_seed_test.go`). No crashers found so far
  - [x] `FuzzDecodePackBits` is now seeded from real RLE payloads (2026-09-18). The comment claiming every fixture was RAW-compressed was stale: six carry composite compression 1 (`gray8-flat`, `rgb8-flat-rle`, `rgb8-rle-layers`, `psb-flat`, `near-psd-limit`, `depth16-rejected`). `extractRLERows` takes the first row of each channel and dedupes to 13 seeds. `psdSections` gained `width` and `depth` as well as `channels`/`height`, because the target's second argument is the decoded **byte** width — `depth16-rejected` is 16-bit, so `width` alone would say 16 where the row decodes to 32 bytes
  - [ ] The text-layer fuzz target still runs on synthetic seeds only — the corpus genuinely has no text descriptor, and will not until S.10.7 lands a text fixture. Renamed in S.10.7 to `FuzzParseTextLayerMetadata`: the descriptor parser it used to target moved to `internal/io/descriptor` and is fuzzed there as `FuzzParse`, so what is left in `psd` is the `TySh` envelope — the transform, the two versions and the trailing bounds — which is the part the descriptor package never sees
  - [x] `Fuzz*` now run as real mutation passes in CI, not only as seed execution: a separate parallel `go-fuzz-smoke` job runs all seven targets at `-fuzztime=30s` (six in `internal/io/psd`, plus `internal/io/descriptor`'s `FuzzParse`). Not `-race` — `-fuzz -race` manages ~3.5k execs/s against ~70k, and race coverage of these paths already comes from the existing `-race` step executing every seed
- [x] **Tagged-block padding is now read tolerantly and written the way real readers expect (2026-09-17).** `parseLayerRecordExtra` consumed a pad byte after *every* odd-length block, so an unpadded one desynchronised the loop — the next signature read as `"BIMl"` — and an unpadded odd block that ended a record made `ReadByte` fail and errored out the whole record. It now consumes the byte only when one is there: a pad byte is `0x00`, and no valid signature starts with `0x00`
  - [x] The writer changed with it. `WriteAdditionalLayerInfoBlock` padded odd payloads per the spec's "rounded up to an even byte count", but inside a layer record no available reader agrees: measured on one file written both ways and differing only in that byte, psd-tools 1.19.0 dies (`Invalid signature b'\x008BI'`, then an invalid channel ID) and ImageMagick 6.9.12 refuses it (`maximum channels exceeded`), while both read the unpadded form correctly. psd-tools and pytoshop both write layer-record blocks unpadded themselves. **Image resource blocks are a different section and really are padded** — `WriteImageResource` is unchanged, and a test guards the boundary
  - [x] Live, not theoretical: `AgAJ` carries a JSON payload of arbitrary length, so an adjustment layer wrote a 63-byte block plus a pad byte. That one survived psd-tools only because `AgAJ` happened to be the record's last block, so the stray byte ended the loop instead of being read as a signature. Every other block Agogo writes is even-length, which is why no fixture caught it
  - [x] Unrelated observation from the same session, recorded so it is not re-diagnosed as a padding bug: ImageMagick refuses **any** file containing a record with zero channels — group dividers and adjustment layers — at `psd.c/ReadPSDLayersInternal`. It refuses the pytoshop-authored group fixtures identically, so it is a limitation of that reader rather than something Agogo emits wrongly
...
- [x] Add explicit truncation tests: every fixture cut at many offsets must produce an error or partial-with-warnings result, never a panic — **done 2026-09-18** over all 22 external fixtures, ~9,900 cuts
  - [x] Every truncation offset of a valid synthetic minimal PSD is covered
  - [x] The sweep is budgeted rather than exhaustive: every offset in a dense prefix, every section boundary from `splitSections` and each ±1, then an even stride to a per-fixture budget. Exhaustive every-offset cost 14.08 s under `-race` (~115k `Parse` calls; `near-psd-limit` alone is 785 ms because it is 30000x2x3); the budgeted sweep adds ~1.4 s to the CI `-race` run, and fixtures smaller than the budget still get every offset because the stride collapses to 1
  - [x] The assertion is contract-only — error, or a result carrying at least one warning; a clean warning-free accept of a truncation fails. No error strings, so parser bounds can move freely
  - [x] **Finding:** across all 22 fixtures the partial-with-warnings branch is currently *unreachable* — every proper truncation errors outright, zero recovered panics, and only `offset == len` parses clean (`depth16-rejected` correctly errors even at full length). The branch is kept because it is legitimate and a future bounds change could make it reachable
  - [x] **New repo convention, introduced here:** `testing.Short()` narrows the sweep, it never skips. A `-short` run is a weaker form of the same assertion, not a different one. The repo had zero `testing.Short` usages before this; nothing in CI or the justfile passes `-short` yet, so it is for local iteration

**Finding the audit produced (2026-09-18) — this phase previously claimed the bound was complete, and it was not:**

- **Layer geometry reached three allocations without ever being validated.** `parseLayerRecord` computed `Bounds.W/H` as raw `int32` corner subtractions with no check against `PSDMaxDimension`/`PSBMaxDimension`/`maxPSDDecodedByteSize`, and `parseChannelImageData` sized `make([]int, height)` and `make([]byte, 0, width*height)` straight from them. A record declaring `bottom-top ≈ 2^31` allocated ~17 GB **before reading a single count byte**, and `Parse`'s `recover()` cannot catch an OOM. `ParseCompositeImageData` had been routed through `checkedPixelCount` and the 512 MiB limit; `parseChannelImageData` called neither.
  - Fixed: corners are subtracted in `int64` so `left = MinInt32, right = MaxInt32` cannot wrap to a small extent, and the result goes through a new `validateLayerGeometry` that delegates the area check to the existing `checkedPixelCount` rather than duplicating overflow math.
  - The same fix closes the **mask** path, which is easy to miss: channel `-2` substitutes `LayerMaskBounds.W/H` for the layer bounds, and `parseLayerMaskData` returns nothing so it cannot reject anything. Validating at the consumer was the only place that covers it without a signature change. Rejecting malformed mask geometry at parse time still needs that signature change — left open.
  - The composite RLE count table had the same shape: `make([]int, header.Channels*header.Height)` was header-driven only, so a 26-byte PSB header (`height=300000, channels=56, width=1`) passed `validateHeaderBounds` and allocated ~134 MB before consuming input. Now bounded against remaining input.
  - The decoded-plane capacity hint is `min(pixelCount, remaining*64)` — PackBits expands at most 128 bytes per 2 input bytes, so the encoded remainder is a real cap and hostile geometry can no longer reserve the full 512 MiB up front.
...
- [x] **`iOpa` fill opacity — done 2026-09-17.** Parsed into `LayerRecord.FillOpacity`, applied on the raster and group import paths, emitted from `buildLayerExtraBlocks`, and covered by `rgb8-fill-opacity`. This closes the last non-descriptor capability in the S.10.1 matrix
  - [x] Written only below 255 (absence means 255 to every reader checked here) and as **four** bytes, not one: psd-tools reads `iOpa` as `B3x` — a byte plus three pad bytes — and only falls back to a bare byte after logging a read error, and it writes the four-byte form itself. An even length also keeps the block clear of the padding disagreement below
  - [x] pytoshop gotcha, confirmed: `GenericTaggedBlock.data`'s **setter is a no-op** — it validates `isinstance(val, bytes)` and then never assigns. The generator passes `data` to the constructor and asserts that it stuck, so an empty block cannot be written silently
  - [x] **Finding — groups ignore fill opacity when rendering.** `compositeLayerOnto`'s `*GroupLayer` branch uses `effectiveLayerOpacity` only and never applies `GroupLayer.FillOpacity()`, while psd-tools' compositor applies it to groups too. Import and export now carry the value faithfully, so this is a renderer gap, not an I/O one — and it is why `rgb8-fill-opacity` has no group in it: a group there would fail `compositePixels` on a difference that has nothing to do with the block being read. The group leg is covered by `TestPSDRoundTripPreservesFillOpacity`, which asserts the model value instead
...
- [x] **Parse the genuine adjustment blocks into live `AdjustmentLayer`s — done 2026-09-17** for all 12 named here: `levl`, `curv`, `hue2`, `brit`, `blnc`, `blwh`, `mixr`, `phfl`, `selc`, `thrs`, `post`, `nvrt`. New `internal/io/psd/adjustments.go` reconstructs each into the engine's existing adjustment kind plus a JSON parameter object, and `psdimport` builds a `model.AdjustmentLayer` before the raster branch is tried. No model change was needed: `AdjustmentLayer` is already `AdjustmentKind` + an opaque `json.RawMessage`. **The solid/gradient/pattern fill types are excluded and stay open** — see the new item below; the engine has no fill-layer type at all, and inventing one is a model decision, not a parser one
  - [x] **The silent loss this closes was worse than "dropped".** A Photoshop adjustment layer has a zero rect, and `flattenLayerPixels` returns `(nil, nil)` for a zero-area rect rather than an error — so the layer arrived as an **empty pixel layer** with the right name and blend mode and nothing else, and `levl`/`curv`/`hue2` produced no warning at all because they were already in the recognised switch. A layer that looks imported and is not is the worst of the three outcomes; pinned by `TestZeroAreaRecordWithNoReconstructionIsReported`
  - [x] **`brit` is obsolete; `CgEd` carries the live values.** psd-tools maps its `BrightnessContrast` layer to `CgEd` and marks `brit` obsolete, and Photoshop writes both. The reader prefers the descriptor in either block order, and `rgb8-adjustment-descriptor` writes the two with deliberately different numbers (+11/-7 vs +30/-20) so a reader that takes the legacy block fails rather than passing by coincidence
  - [x] Values with no engine home are reported, never rounded into a field that means something else: `levl`'s per-channel records (the engine holds one set of level values), `mixr`'s constant and fourth source weight, `hue2`'s four movable hue edges, `blwh`'s tint colour when it is absent. Two blocks are refused outright rather than reconstructed wrongly — `curv` version 4, which drops the channel bitmap so a red curve could only be guessed onto the composite, and a `phfl` in a non-RGB colour space, whose components are not RGB values at all
  - [x] **Four correctness defects caught in PR review (2026-09-18), all of them cases where a partial reading was worse than none.** `hue2`'s mode byte was read inverted *and* the colorize triple was ignored, so every colorized layer took the wrong three numbers — psd-tools' own compositor settled both, and the fixture now carries a colorized/non-colorized pair whose triples differ in every component. `mixr` decoded only the first output row, and `channelMixerAdjustmentFactory` has no defaulting, so the imported layer zeroed the green and blue outputs; all rows are read now, an absent row becomes identity rather than zero, and monochrome replicates its single grey row because the engine takes the luminance of the full matrix. `phfl` built an RGB colour out of CMYK or Lab components while only warning about it. `blwh` imported the tint flag without the tint colour, leaving the engine's fixed brown in its place
  - [x] Fuzzed: `FuzzParseAdjustmentBlock` drives all 12 parsers with the same bytes and asserts that a refusal leaves no half-built adjustment behind — 5.17M execs, no crashers; seeded from the new fixtures and added to the CI `go-fuzz-smoke` job
- [ ] Give solid/gradient/pattern fill layers (`SoCo`/`GdFl`/`PtFl`) a real model type. Split out of the item above (2026-09-17): the engine has no fill-layer type — `Fill` and `Apply Gradient` bake into a `PixelLayer`, and the colour/gradient/pattern **overlays** are properties of another layer rather than layers in the stack. They currently import with a warning naming the block; reconstructing them needs a new layer type, the compositor and the project archive, which is a model decision rather than parser work
- [x] **Fixed (2026-09-18, from PR review) — the adjustment compositor ignored opacity and blend mode.** `applyAdjustmentLayerToSurface` takes its coverage from the clip alpha and the layer mask only; `Opacity()` and `BlendMode()` are never read, so a visible adjustment layer at opacity 160 with Multiply renders full-strength Normal. This is a **renderer** gap, not an I/O one — the same shape as the group fill-opacity finding above — and it predates PSD import: a layer created in the UI has always behaved this way. Import carries both values faithfully, and the fixture's Levels layer pins them at model level. Now fixed: the blend runs through the same `agg_go` `CompositeImage` every other layer uses, fed two opaque images so that source-over reduces to `lerp(dest, blend(dest, src), opacity)`. The merge back is deliberately **not** a composite — an adjustment recolours the backdrop and adds no coverage, so source-over would raise a half-transparent backdrop from 0.5 to 0.75 alpha. Normal at 100% keeps the original loop (the S.4 hot path) and is pinned against the general path by a test. `AdjustmentCache` gained `Opacity`/`BlendMode`, since it stores the surface *after* the merge. The render tests were verified to fail against the old behaviour
- [ ] Export adjustment layers natively — the writer half of the item above. `psdexport` still emits the private `AgAJ` block, so an Agogo-written adjustment layer re-imports as a pixel layer. Every affected assertion is recorded in the three fixtures' `writer.lossy` allowlists with a reason, and confirmed from the outside: psd-tools reads the re-exports back as `kind=pixel`
- [x] **Shared descriptor reader extracted — done 2026-09-17.** `internal/io/descriptor` now holds one bounded, recursive Action Descriptor parser for both ABR and PSD. `internal/io/abr` keeps its public names as **type aliases**, so every existing consumer compiled unchanged and the ABR suite passed without edits — which is the whole justification for the alias over a copy. `psd.ParseDescriptorTextValue` is gone; `parseTextLayerMetadata` reads the whole `TxLr` descriptor instead of scanning for one key
  - [x] **The old PSD reader could not have read a real text layer.** It understood only `TEXT`, `bool`, `doub` and `long` and returned an error on every other OSType, so the first `Objc`, `enum` or `tdta` in a Photoshop `TxLr` aborted the parse and the layer fell through to the flatten path. `TestParseTextLayerMetadataReadsTextPastRicherValueTypes` pins the fix
  - [x] **Defect found in the ABR parser while moving it:** `type`/`GlbC`/`Clss` values were read as a bare class ID, but the format is a unicode **name** followed by a length-and-key class ID. Every value after one desynchronised. Fixed, and pinned in both packages by a test whose detector is a trailing `bool` that is only reachable if the class value consumed exactly its own bytes — verified to fail against the old reading
  - [x] **Hostile-count bound carried across, and it was not a formality.** The ABR parser bounded a declared item count only against `MaxDescriptorItems` (100k) and then reserved the slice; the PSD side already bounded against remaining input. The shared parser applies the remaining-input bound **first**, because a count the file cannot contain is a structural impossibility while the budget is a policy cap. Measured: without it, a **12-byte payload allocates 20 MB**. The test asserts allocated *bytes*, not `testing.AllocsPerRun` — one `make([]Item, 0, n)` is a single allocation at any size, so a count-based assertion passes while reserving megabytes
  - [x] `ObAr`, `obj `, `UnFl` and the reference types stay unimplemented and error naming the type rather than being skipped: a descriptor value carries no length prefix, so a reader that does not understand one cannot step over it. Neither `lfx2` nor `TySh` uses any of them
  - [x] Fuzzed: `descriptor.FuzzParse` (6.7M execs) and `psd.FuzzParseTextLayerMetadata` (4.1M execs), no crashers; both added to the CI `go-fuzz-smoke` job
...
- [x] **Fallback contract defined and phrased by outcome — done 2026-09-17.** Every recognised block that did not become engine state now produces a warning naming it, and the message says what actually happened to the layer: *imported as a flattened pixel layer*, *not imported; the layer was reconstructed from another block*, *not imported; the group itself was preserved*, or *not imported; the layer carried no pixels and was dropped*. The old single message claimed "imported as flattened pixel layer" for all of them, which was wrong in three ways — an adjustment layer had no pixels to flatten, a layer whose channels were missing was dropped outright while the message said "imported", and a block the parser *did* recognise said nothing at all. Groups also stopped dropping their `UnsupportedBlocks` silently. Pinned by `TestImportWarnsForEveryUnreconstructedBlock`
  - [x] `AgAJ`, Agogo's own private block, is reported like any other. It is not a PSD feature, and a file relying on it is relying on something no other application can read — which is the argument for the open item above it
- [x] **Smart objects stay out of scope, with an accurate warning — done 2026-09-17.** The generic wording understated the loss in one direction and overstated it in the other; the message now says the flattened composite *was* imported and the placed source document, its transform and its smart filters were not. Pinned by `TestImportSmartObjectWarningNamesWhatWasLost`
- [ ] Add fixture tests per adjustment type, per text-layer style variation, and per effect, asserting the reconstructed layer's parameters — not just that some layer appeared — (2026-09-17) — partial: the adjustment leg is done. `LayerExpect` gained `adjustmentKind` and `adjustmentParams`, compared key by key so a failure names the parameter rather than dumping JSON, and `derive_expectations.py` maps psd-tools' parsed structures onto the engine's parameter vocabulary as an independent second implementation. The text and effect legs wait on their reconstruction work above
