...
Status: reviewed after the Phase S.9 compositor, viewport, gradient, and crop migrations (2026-08-02); adjustment blending revisited in S.10.7 (2026-09-18).
...
| Destination-dependent decisions | `eraseBackgroundDabResource` (`EraseBackgroundDab`), `applyBlendIfChannelsClipped`, `applyAdjustmentRectBlended` | Tolerance erase must inspect the existing destination colour before deciding whether to erase. BlendIf channel gating conditionally restores individual channels and is not an AGG comp-op. `applyAdjustmentRectBlended` runs its blend through `CompositeImage`; the surviving loop exists because an adjustment layer RECOLOURS the backdrop rather than covering it, so the result must keep the backdrop's alpha. A source-over composite would raise a half-transparent backdrop from 0.5 to 0.75 alpha, adding coverage the adjustment never had, and no comp-op expresses "blend the colour, leave the coverage". |
